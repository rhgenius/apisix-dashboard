...
build: web-default api-default
	api/build.sh && cd ./web && export CYPRESS_INSTALL_BINARY=0 && yarn config set strict-ssl false && yarn install --ignore-scripts && yarn config set strict-ssl true && yarn build  && mkdir -p ../output/logs
